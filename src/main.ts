import './style.scss'

// Interfaces TypeScript
interface ServiceUrls {
  gestao: string;
  solicitacoes: string;
  handson: string;
}

interface DashboardData {
  suasSolicitacoes: number;
  imoveisAtivos: number;
  solicitacoesDaUnidade: number;
}

// URLs dos sistemas integrados
const URLS: ServiceUrls = {
  gestao: 'https://osvaldojeronymo.github.io/silic-input-doc/?from=portal',
  solicitacoes: 'https://osvaldojeronymo.github.io/silic-request-service/?from=portal',
  handson: 'https://osvaldojeronymo.github.io/silic-hands-on/?from=portal'
};

/**
 * Navega para um sistema específico
 * @param servico - Nome do serviço (gestao, solicitacoes ou handson)
 */
function navegarPara(servico: keyof ServiceUrls): void {
  console.log('Navegando para:', servico);
  
  if (URLS[servico]) {
    window.open(URLS[servico], '_blank');
  } else {
    console.error('Serviço não encontrado:', servico);
  }
}

/**
 * Executa ações rápidas
 * @param tipo - Tipo da ação (cadastro, busca, relatorio)
 */
function acao(tipo: 'cadastro' | 'busca' | 'relatorio'): void {
  console.log('Ação:', tipo);
  
  switch(tipo) {
    case 'cadastro':
      // Vai diretamente para a seção de cadastro no sistema de gestão
      window.open(URLS.gestao + '#cadastro', '_blank');
      break;
      
    case 'busca':
      // Vai para a seção de busca/SIPGE no sistema de gestão
      window.open(URLS.gestao + '#sipge', '_blank');
      break;
      
    case 'relatorio':
      // Placeholder - em desenvolvimento
      alert('Funcionalidade de relatórios em desenvolvimento.\n\nEm breve você poderá:\n• Gerar relatórios de imóveis\n• Exportar dados\n• Visualizar dashboards');
      break;
      
    default:
      console.error('Ação não reconhecida:', tipo);
  }
}

/**
 * Atualiza os números do dashboard
 * Esta função pode ser expandida para buscar dados reais de uma API
 */
function atualizarDashboard(): void {
  // Placeholder - em produção, estes dados viriam de uma API
  const dados: DashboardData = {
    suasSolicitacoes: 3,
    imoveisAtivos: 100,
    solicitacoesDaUnidade: 8
  };
  
  // Atualiza os elementos na tela
  const elementos = document.querySelectorAll('.status-number');
  if (elementos.length >= 3) {
    (elementos[0] as HTMLElement).textContent = dados.suasSolicitacoes.toString();
    (elementos[1] as HTMLElement).textContent = dados.imoveisAtivos.toString();
    (elementos[2] as HTMLElement).textContent = dados.solicitacoesDaUnidade.toString();
  }
}

/**
 * Adiciona interações e animações
 */
function adicionarInteracoes(): void {
  // Adiciona hover effects nos cards
  const cards = document.querySelectorAll('.service-card') as NodeListOf<HTMLElement>;
  cards.forEach(card => {
    card.addEventListener('mouseenter', function() {
      this.style.transform = 'translateY(-8px)';
    });
    
    card.addEventListener('mouseleave', function() {
      this.style.transform = 'translateY(0)';
    });
  });
  
  // Adiciona hover effects nos botões rápidos
  const quickBtns = document.querySelectorAll('.quick-btn') as NodeListOf<HTMLElement>;
  quickBtns.forEach(btn => {
    btn.addEventListener('mouseenter', function() {
      this.style.transform = 'translateY(-3px)';
    });
    
    btn.addEventListener('mouseleave', function() {
      this.style.transform = 'translateY(0)';
    });
  });
}

/**
 * Configura event listeners para os botões
 */
function configurarEventListeners(): void {
  // Botões de serviços
  const serviceButtons = document.querySelectorAll('.btn-service') as NodeListOf<HTMLElement>;
  serviceButtons.forEach(button => {
    const service = button.getAttribute('data-service') as keyof ServiceUrls;
    if (service) {
      button.addEventListener('click', () => navegarPara(service));
    }
  });

  // Botões de ações rápidas
  const quickButtons = document.querySelectorAll('.quick-btn') as NodeListOf<HTMLElement>;
  quickButtons.forEach(button => {
    const url = button.getAttribute('data-url');
    if (url) {
      button.addEventListener('click', () => {
        window.open(url, '_blank');
      });
    }
  });
}

/**
 * Inicialização quando a página carrega
 */
document.addEventListener('DOMContentLoaded', function() {
  console.log('Portal SILIC 2.0 carregado!');
  
  // Configura event listeners
  configurarEventListeners();
  
  // Atualiza o dashboard
  atualizarDashboard();
  
  // Adiciona interações
  adicionarInteracoes();
  
  // Log para debug
  console.log('Sistemas disponíveis:', Object.keys(URLS));
});

// Exporta funções para uso global (para compatibilidade)
(window as any).navegarPara = navegarPara;
(window as any).acao = acao;
